...
static  int S500ValidGpio [132] =   // Modified for sparky
{
    ...
    -1, -1, -1, -1, -1, -1, -1, -1, 120, 121, -1, -1, -1, -1, -1, -1, //112-> 127  //SK MOD
...
int main (int argc, char *argv [])
{
    ...
    if (strcmp (argv [1], "-v") == 0)
    {
        ...
        else
        {
            if (1)
            {
                printf ("Sparky Details:\n") ;
                printf ("  Type: %s, Revision: %s, Memory: %dMB, Maker: %s %s\n",
                        piModelNames [model], piRevisionNames [rev], mem, piMakerNames [maker], overVolted ? "[OV]" : "") ; // jk TODO  mem?, overVolted?
            }
            else
            {
